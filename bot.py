...
# ----------------------------
# /Button -> Broadcast flow (private only, for @Stiff_OWi only)
# ----------------------------
...
async def bc_menu_click(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data != "bc_broadcast":
        return BC_MENU

    context.user_data["bc"] = {"title": "", "text": "", "file": None}
    await query.edit_message_text("Введи заголовок рассылки (он будет жирным):")
    return BC_TITLE
...
async def bc_confirm_click(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "bc_attach":
        await query.edit_message_text("Ок! Отправь файл (документ/фото/видео). Или напиши /skip чтобы без файла.")
        return BC_FILE

    if query.data == "bc_send":
        chat_id = get_target_chat_id()
        if not chat_id:
            await query.edit_message_text("Чат не назначен. Сначала /setchat в группе.")
            return ConversationHandler.END

        bc = context.user_data.get("bc", {})
        title = bc.get("title", "")
        text = bc.get("text", "")
        payload = f"<b>{escape_html(title)}</b>\n{escape_html(text)}"
        ...
        try:
            ...
            await query.edit_message_text("✅ Отправлено в группу.")
        except Exception:
            log.exception("Broadcast send failed")
            await query.edit_message_text("❌ Не смог отправить. Проверь права бота в группе.")
        finally:
            context.user_data.pop("bc", None)
    ...
    if prev is not None and (now - prev) <= SPAM_WINDOW_SECONDS:
        ...
        if (now - last_warn) >= SPAM_COOLDOWN_SECONDS:
            if u == SERJO_NICK.lower():
                warn = random.choice(SERJO_SPAM_WARNINGS)
                await msg.reply_text(f"@{SERJO_NICK} {warn}")
            else:
...
async def send_serjo_day(app: Application) -> None:
    ...
    if not chat_id:
        return
    text = build_from(SERJO_REMINDERS, SERJO_NICK)
...
async def send_serjo_night(app: Application) -> None:
    ...
    if not chat_id:
        return
    text = build_from(SERJO_NIGHT, SERJO_NICK)
    await app.bot.send_message(chat_id=chat_id, text=text)
...
async def send_chernov_hourly(app: Application) -> None:
    ...
    if not chat_id:
        return
    text = build_from(CHERNOV_REMINDERS, CHERNOV_NICK)
    await app.bot.send_message(chat_id=chat_id, text=text)
...
def main() -> None:
    ...
    # Private file_id flow
    app.add_handler(MessageHandler(filters.PHOTO, handle_photoid_photo))

    # Anti-spam in group
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_text))

    # /Button conversation (private only + only for BUTTON_ADMIN)
    ...
    scheduler.add_job(
        send_chernov_hourly,
        trigger=CronTrigger(hour="9-21", minute=0, timezone=TZ),
        kwargs={"app": app},
        id="chernov_hourly",
        replace_existing=True,
    )
    ...
    scheduler.start()
    log.info("Scheduler started (MSK): serjo 09-19/2 + 21, chernov hourly 09-21")
